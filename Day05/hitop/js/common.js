/*
    ...
    */
    ...
    //     mapID:"map",
    //     title:"(주) 하이탑"});
    // makeMap({mapID:"map",address:"강원도 동해시 동굴로 147",title:"강원도 사무실"});
    // makeMap({mapID:"map",address:"경기도 남양주시 화도읍 비룡로 464번길 42",title:"제 1 공장"});
    // makeMap({mapID:"map",address:"경기도 파주시 광탄면 명봉산로 352번길 34-54",title:"제 2 공장"});
    let mapInfo=[
        {
            address:"서울 송파구 성내천로 14길 13",
            mapID:"map",
    ...
    $("#location .tab .menu li").on("click",function(){
        let sel = $(this).index();
        $("#map").html("");
        makeMap({address:mapInfo[sel].address,mapID:mapInfo[sel].mapID,title:mapInfo[sel].title})    
    });